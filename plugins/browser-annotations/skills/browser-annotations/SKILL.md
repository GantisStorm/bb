...
- Saved notes survive navigation within the same Browser tab. Each note retains its source URL and captured element metadata. `get`, text `export`, `copy`, and `add-to-chat` operate on the combined collection in its current order; use the tab's current target revision even when editing an older note. Navigation clears unfinished page-specific reviews, pickers, and screenshot-editor state, not saved notes. Remove notes explicitly or use `clear-notes`; closing the tab or removing its thread/environment clears the collection.
